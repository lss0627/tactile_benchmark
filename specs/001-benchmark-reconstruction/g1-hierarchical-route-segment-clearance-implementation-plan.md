...
- [ ] Assert route completeness for all six classes, all five exact commands,
...
- [ ] Assert complete-polyline bounds for endpoint-safe/middle-unsafe,
  reversal, non-monotonic, remainder, revolute, prismatic, and malformed
  kinematic fixtures.
- [ ] Assert sphere and AABB lower-bound soundness against exact reference
  distances for safe/unsafe/boundary/nextafter/offset/inflation/nonfinite
  fixtures.
- [ ] Assert deterministic root certification, midpoint split, left-before-
...
- [ ] Assert 17 x 2 all-pair coverage and reject missing, extra, duplicate,
  reordered, or unknown collider identities.
- [ ] Assert exact geometry digest reuse, independent lifecycle bindings,
  mutation rejection, cached-payload digest rejection, and Python object-ID
  irrelevance.
- [ ] Run the exact node and require behavior assertions caused by missing
  APIs only:
...
- [ ] Run the three exact integration nodes and require only approved behavior
  assertion failures:
...
- [ ] Commit the unchanged-node-inventory RED as
...
- [ ] Define frozen `RouteMicroSegment`, `RouteAction`, `RouteBlock`,
...
- [ ] Implement `materialize_route_micro_segments` with exact 256-action,
  two-segment, identity, float64-byte, cadence, stopping, motif, and shared-
  kernel validation.
- [ ] Implement canonical JSON/digest helpers that exclude only the record's
  own digest field and reject nonfinite values or arbitrary objects.
- [ ] Implement strict validators for action partition, block partition,
  collider pair product, record digest, and no-claim truth fields.
- [ ] Expose the existing stopping-target calculation from
  `g1_full_robot_clearance.py` as the only stopping model used by route
  materialization; do not duplicate its formula.
- [ ] Run the focused route-completeness assertions to GREEN while broadphase
  and integration assertions remain RED.
- [ ] Commit as `fix(g1): materialize immutable route segments`.
...
- [ ] In `g1_full_robot_clearance.py`, prepare validated numerical collider
  ...
  states.
- [ ] Keep shape support, geometry type interpretation, joint graph, offsets,
  and exact transforms owned by `g1_full_robot_clearance.py`; test fixtures do
  not become production authority.
- [ ] In `g1_route_segment_clearance.py`, implement the complete-polyline
  motion sum over every ordered micro-segment and every moving ancestor.
- [ ] Implement revolute chord and prismatic variation terms exactly as fixed
  in the architecture. Retain each per-joint/per-segment contribution in the
  diagnostic digest.
- [ ] Implement enclosing-sphere geometry/solid/effective lower bounds.
- [ ] Implement reference-AABB expansion by the scalar full-block motion bound
  and Euclidean axis-gap geometry/solid/effective lower bounds.
- [ ] Require strict `solid > 0.0 and effective > 0.0`; do not add epsilon,
  `isclose`, or rounding.
- [ ] Fail with `G1_FULL_ROBOT_ROUTE_BLOCK_UNRESOLVED` for any unknown,
  nonfinite, negative, inverted, incomplete, or digest-inconsistent input.
- [ ] Run all motion/broadphase reference assertions to GREEN.
- [ ] Commit as `fix(g1): certify conservative route blocks`.
...
- [ ] Implement root `[0,256)` evaluation for every canonical subject/
  obstacle pair.
- [ ] Implement deterministic half-open midpoint splitting and left-before-
  right traversal with no sorting, set repair, remainder loss, or adaptive
  command scaling.
- [ ] Define the one-action leaf as both existing segments and delegate it to
  an injected `exact_action_certifier` created by
  `g1_full_robot_clearance.py` around `certify_articulated_sweep`.
- [ ] Memoize at most one canonical exact full-action receipt per action; on
...
- [ ] Propagate existing unsafe, unresolved, Contact, provenance, cache, and
  work-budget failures without changing codes or messages.
- [ ] Build a deterministic block-tree digest and disjoint ordered pair
  coverage partition; incomplete coverage fails closed.
- [ ] Run split, exact-leaf, all-pair, adversarial, and no-false-safe
  assertions to GREEN.
- [ ] Commit as `fix(g1): delegate unresolved route leaves`.
...
- [ ] Implement `build_geometry_equivalence_record` over the complete geometry,
  offset, articulation, selected-pose, route, cadence, stopping, phase, and
  proof-policy fields fixed by the architecture.
- [ ] Exclude scene-local lifecycle values, Python object identity, timestamps,
  and memory addresses from the geometry-equivalence digest.
- [ ] Implement bounded `RouteProofCache` with exact key and value digests,
  detached values, LRU statistics, mutation detection, key-rebinding failure,
  and no safety fallback on eviction.
- [ ] Require each fresh scene to validate its own snapshot, offset authority,
  stage/articulation/latch lifecycle, and geometry-equivalence record before a
  pure proof can be reused.
- [ ] Emit a separate lifecycle binding receipt that references the reused
  proof digest but never copies a prior lifecycle token.
- [ ] Run cold/warm/mutated and same/different-geometry cross-scene assertions
  to GREEN.
- [ ] Commit as `fix(g1): bind reusable route proofs to geometry`.
...
- [ ] Refactor `certify_option_d_preliminary_route_diagnostics` into two
  explicit phases: materialize all authoritative governed/stopping states, then
  certify one route proof per class/command.
- [ ] Preserve the initial exact sweep so the full work plan remains 7,681-
  sweep equivalent.
- [ ] Replace 256 immediate exact calls per route with one hierarchical proof;
  only unresolved action leaves enter the exact authority.
- [ ] Record v3 route diagnostics, route proof digests, block/certificate/split/
  leaf counts, all-pair coverage, lower-bound labels, proof cache counters, and
  existing work ledger.
- [ ] Extend write-ahead progress with route materialized, block milestone,
  leaf fallback, route proof retained, completion, and failure events while
  preserving the existing 4,096-record ceiling.
- [ ] Write route proofs and lifecycle binding receipts before classification
  and unique shutdown. Writer failure retains the sibling journal and cannot
  emit a valid manifest/checksum file.
- [ ] Preserve selected cap null, claim eligible false, no actuation/readiness,
  post-abort zero, and force/wrench/raw-impulse false for preliminary data.
- [ ] Run the exact static runtime and writer-lifecycle RED nodes to GREEN.
- [ ] Commit as `fix(g1): integrate hierarchical C2a route proof`.
...
- [ ] Require v6/v6.creation-failure scene records, v3 route diagnostics, and
  v1 route proof artifacts for newly generated evidence.
- [ ] Continue accepting historical v1-v5 only as immutable/no-claim evidence;
  do not synthesize route proofs or lifecycle bindings for old records.
- [ ] Add route proof, geometry-equivalence, lifecycle-binding, work-ledger,
  and cache fields to report, manifest, checksums, and write-before-close
  validation.
- [ ] Keep existing frozen node IDs by extending current test functions. If a
  new node is mathematically required, record an explicit one-to-one inventory
  migration before updating any approved digest.
- [ ] Verify attempt-09 checksum-file SHA and all historical payloads again.
- [ ] Commit as `docs(g1): migrate hierarchical route proof evidence`.
...
- [ ] Run the existing 100-action reference/optimized equivalence benchmark.
- [ ] Run the full 7,681-sweep-equivalent fixture: one initial exact action plus
  six classes x five commands x 256 public actions, two micro-segments per
  action, 17 subjects x two obstacles.
- [ ] Run adversarial unsafe/boundary/stopping/reversal/non-monotonic fixtures.
- [ ] Run proof-cache cold/warm/mutated and cross-scene same/different geometry
  workloads.
- [ ] Record total actions, micro-segments, blocks, sphere/AABB certificates,
...
- [ ] Require full-plan completion, false-safe zero, unresolved zero for the
  ...
  equivalence.
- [ ] Stop for architecture review instead of changing budgets, inventory, or
...
- [ ] Run T152 113/113, original GREEN 748/748, current GREEN 966/966,
...
- [ ] Review lower-bound mathematical direction, complete-polyline and
...
- [ ] Require `Critical=0` and `Important=0`. Record every Minor finding and
  why it cannot change this stage's result.
- [ ] Fix verified Critical/Important findings with focused RED→GREEN commits,
  then rerun affected and performance gates before closing review.
- [ ] Commit as `docs(g1): review hierarchical route segment clearance`.
...
- [ ] Bind the final implementation and review commits without writing the
  projection's unknown SHA into its own contents.
- [ ] Record complete RED→GREEN, full-plan performance, regression inventory,
  ...
  unvalidated.
- [ ] Run `git diff --check`, unresolved-marker scan, document consistency audit,
  and historical checksum verification.
- [ ] Commit and push as
  `docs(g1): project hierarchical route segment clearance`.
- [ ] Require a clean worktree and local/tracking/live-origin/PR-head equality.
