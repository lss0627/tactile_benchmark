...
**Status:** `TASK_7_GREEN_BLOCKED_PENDING_ARCHITECTURE_APPROVAL`
...
GREEN is not implementable under the current combined contract.
...
One of these mutually exclusive seams must be approved before Task 7 GREEN:
...
1. **Separate geometry authoring from complete stage construction
   ...
   This requires revising the approved fake test from
   `build_stage(..., authoring_adapter=fake)` to the explicitly geometry-only
   seam.
