#!/usr/bin/env python
"""Check local seeded FR3 IK targets without sending joint commands."""
...
from isaac_tactile_libero.robots.fr3_ik_controller import (  # noqa: E402
    ...
    build_dry_local_ik_target_report,
    solve_local_ik_actions,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="outputs/fr3_ik_safety_refinement/local_ik_target_report.json")
...
def run_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        report = solve_local_ik_actions(runtime=runtime, mapping_config=mapping, safety=safety, joint_state=joint_state)
        report.update(
            {
...
def main() -> int:
    args = parse_args()
    if args.dry_run:
        report = build_dry_local_ik_target_report()
        report.update(
            {
                "robot_config_path": str(args.robot_config),
                "controller_config_path": str(args.controller_config),
