#!/usr/bin/env python
"""Run one tiny FR3 EE delta using an IK joint target.

This script never connects a task, never collects datasets, and never produces
benchmark results. Dry-run mode does not import or start Isaac Sim.
"""
...
from isaac_tactile_libero.robots.fr3_ik_controller import (  # noqa: E402
    ...
    TINY_EE_DELTA_1MM_ACTION,
    TINY_EE_DELTA_ACTION,
    build_dry_ik_motion_status,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        choices=("tiny_ik_ee_delta", "tiny_ik_ee_delta_1mm"),
        default="tiny_ik_ee_delta",
    )
    parser.add_argument("--output", default="outputs/fr3_ik_controller_probe/tiny_ik_ee_delta_status.json")
...
def screenshot_path_for_output(output_path: str | Path) -> Path:
    return Path(output_path).with_name("fr3_ik_ee_motion_smoke.png")
...
def run_runtime(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        action = TINY_EE_DELTA_1MM_ACTION if args.mode == "tiny_ik_ee_delta_1mm" else TINY_EE_DELTA_ACTION
        result = runtime.run_tiny_ik_ee_delta(
...
def main() -> int:
    args = parse_args()
    if args.dry_run:
        status = build_dry_ik_motion_status(mode=args.mode)
        status.update(
            {
                "robot_config_path": str(args.robot_config),
                "controller_config_path": str(args.controller_config),
